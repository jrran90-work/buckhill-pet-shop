  ...
  <v-container fluid>
    ...
    <v-row class="mt-16">
      <v-col cols="6" offset-md="3">
        <h2 class="text-h2 primary--text mb-9">{{ pageData.title | capitalize }}</h2>
        <v-sheet>
          <v-row>
            <v-col cols="12" md="9">
              <v-breadcrumbs class="breadcrumb-wrapper px-2 black--text" :items="setPageCrumbs(pageData)"></v-breadcrumbs>
            </v-col>
            <v-col cols="12" md="3">
              <v-select 
...
export default {
  ...
  computed: {
    ...
    pageData () {
      return this.$route.params.type === 'brand'
        ? this.brand
        : this.category;
    },
  },
  async mounted () {
    if (this.$route.params.type === 'brand') {
      this.typeParams = { brand: this.$route.params.uuid }
      await this.$store.dispatch('brand/getBrand', this.$route.params.uuid)
    } else if (this.$route.params.type === 'category') {
      this.typeParams = { category: this.$route.params.uuid }
      await this.$store.dispatch('category/getCategory', this.$route.params.uuid)
    }
  ...
  methods: {
    setPageCrumbs (page) {
      ...
      const crumbs = ([
        ...
        { exact: true, text: 'Products', to: { name: 'products' } },
        {
          exact: true,
          text: this.$options.filters.capitalize(this.$route.params.type),
          to: typeRoute
        },
        {
          exact: true,
          text: this.$options.filters.capitalize(page.title),
          disabled: true
        },
      ])
